...
knife ec2 server create -N "CC Chef Server" -x ubuntu -I $AMI_ID  -f $INSTANCE_TYPE -k $KEY_NAME --ssh-key $KEY_FILE  -G $SECURITY_GROUPS | tee instance_info
...
echo "knife[:identity_file] =\"#{ENV['CHEF_PEM']}\"" >> .chef/knife.rb
